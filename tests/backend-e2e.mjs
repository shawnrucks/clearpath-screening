...
try{
  ...
  const auditRows=db.prepare("SELECT user,role,entity_type,previous_value,new_value FROM cp_audit WHERE id>? ORDER BY id").all(initialAuditId);
  assert.equal(auditRows.length,8);assertions++;
  for(const row of auditRows){check(row.user&&row.role&&row.entity_type,"audit actor/entity required");if(row.entity_type!=="Operations Report"){check(JSON.parse(row.previous_value),"audit previous snapshot required");const evidence=JSON.parse(row.new_value);check(evidence.record&&evidence.submittedFields,"audit new evidence required")}}
